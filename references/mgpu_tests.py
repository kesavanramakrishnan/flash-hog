...
def matmul4(a, b, config: TuningConfig):
    ...
    def kernel(
        ...
        def _mn_loop(loop_info: plgpu.NDLoopInfo):
            ...
            def _per_warp():
                ...
                def _memory():
                    def _loop_body(ki, _):
                        ...
                        @pl.when(
                            jnp.logical_or(
                                ki >= max_concurrent_steps, loop_info.local_index > 0
                            )
                        )
                        def _():  # Make sure the data has been consumed before overwriting.
                            plgpu.barrier_wait(consumed_barriers.at[slot])
            ...
            plgpu.wait_smem_to_gmem(
                0, wait_read_only=True
            )  # Make sure that previous store is done.
            plgpu.barrier_wait(mma_done_barrier)
            out_m_index = m_index * 2 + lax.axis_index("cluster")
            out_m_slice = pl.ds(out_m_index * tile_m, tile_m)
            out_gmem_window = out_gmem.at[out_m_slice, n_slice]
...
def matmul5(a, b, config: TuningConfig):
    ...
    def kernel(
        ...
        def _mn_loop(loop_info: plgpu.NDLoopInfo):
            ...
            mn_acc_tmem = acc_tmem.at[
                :, pl.ds(acc_slot * cluster_tile_n, cluster_tile_n)
            ]
            ...
            def _store_wg():
                ...
                for ni in range(cluster_tile_n // config.epilogue_tile_n):
                    acc_smem_ni = acc_smem.at[ni % 2]
                    ni_slice = pl.ds(
                        ni * config.epilogue_tile_n, config.epilogue_tile_n
                    )
                    # Make sure that previous copy is done before we overwrite.
                    plgpu.wait_smem_to_gmem(1, wait_read_only=True)
                    acc_smem_ni[...] = plgpu.async_load_tmem(
                        mn_acc_tmem.at[:, ni_slice]
                    ).astype(dtype)
                    plgpu.commit_smem()
                    plgpu.copy_smem_to_gmem(
                        acc_smem_ni, out_gmem_window.at[:, ni_slice]
                    )
...
class MatmulTutorialTCGen05Test(jtu.JaxTestCase):
    BENCHMARK = False
...
if __name__ == "__main__":
    ...
    #     max_concurrent_steps=4,
    # )
    # a = jrandom.normal(jrandom.PRNGKey(0), (m, k), dtype)
    # b = jrandom.normal(jrandom.PRNGKey(1), (k, n), dtype)
    # out = jax.jit(partial(matmul6, config=example_config))(a, b)
    # print(f"{out.shape=}")
