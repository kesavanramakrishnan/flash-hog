...
def mha_forward_kernel(
    ...
    *residual_refs: Any,  # Residual outputs
    ...
    q = pl.load(q_ref, mask=head_mask, other=0.0)
    ...
    def body(start_k, carry):
        ...
        s_curr = jnp.exp2(qk - m_next[:, None])  # Use m_next instead of m_curr to avoid a correction on l_curr
...
def mha(
    ...
    if (q.shape[-1] != k.shape[-1]) or (q.shape[-1] != v.shape[-1]):
        raise ValueError(f"This kernel expects q, k, and v to have the same head dimension, but found {q.shape=}, {k.shape=}, {v.shape=}.")
    ...
    kernel = functools.partial(mha_forward_kernel, sm_scale=sm_scale, block_q=block_q, block_k=block_k, head_dim=head_dim, causal=causal)
    ...
    if return_residuals:
        out_shape.append(jax.ShapeDtypeStruct(shape=(batch_size, num_heads, q_seq_len), dtype=jnp.float32))  # lse
        out_specs.append(pl.BlockSpec((None, None, block_q), lambda i, j, k: (j, k, i)))  # lse
    out = pl.pallas_call(
        kernel,
        grid=grid_,
        in_specs=in_specs,
...
def mha_backward_kernel(
    ...
    *,
    ...
    plgpu.store(dv_ref.at[:, : dv.shape[-1]], dv.astype(dv_ref.dtype), mask=head_mask)
    plgpu.store(dk_ref.at[:, : dk.shape[-1]], dk.astype(dk_ref.dtype), mask=head_mask)
...
def _mha_backward(
    ...
    if backward_pass_impl == "xla":
        ...
        if q_seq_len // block_q_dq != kv_seq_len // block_kv_dkv:
            raise ValueError("q_seq_len and kv_seq_len must be divided into the same number of blocks for the fused backward pass.")
...
if __name__ == "__main__":
    ...
    q = jrandom.normal(jrandom.PRNGKey(0), (batch_size, q_seq_len, num_heads, head_dim))
    k = jrandom.normal(jrandom.PRNGKey(1), (batch_size, kv_seq_len, num_heads, head_dim))
    v = jrandom.normal(jrandom.PRNGKey(2), (batch_size, kv_seq_len, num_heads, head_dim))
    ...
    out = mha(q, k, v, segment_ids, sm_scale, causal, block_sizes, backward_pass_impl, num_warps, num_stages, grid, interpret, debug, return_residuals)
